...
practices_dates.csv
lsoa_dates.csv
practices_ccgs_dates.csv

categories_prac.csv
categories_lsoa.csv
categories_ccg.csv

OP_measures_prac.csv
OP_measures_ccg.csv
...
## 3. Remove infoabout BNF code subsets that are not used in the analysis
...
## 4. Remove BNF codes not used in the analysis
## TODO: merge this with chunk above?
```{r}


BNF_codes_measures_used_only_short <- BNF_codes_measures_used_only %>%
  filter(
...
write_csv(lsoa_dates, path = paste(output_dir, 'dates_lsoa.csv', sep=''))
...
write_csv(practices_ccgs_dates, path = paste(output_dir, 'dates_practices_ccgs.csv', sep=''))
...
write_csv(categories_ccg, path = paste(output_dir, 'categories_ccg.csv', sep=''))
...
OP_prescriptions %>%
  left_join(
    BNF_codes_measures %>% select(bnf_code, high_dose_ics_products, all_ics_products),
    by = "bnf_code"
  ) %>%
